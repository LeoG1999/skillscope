...
5. 票据品类属于提交人的受限品类时，必须先调用 `manager.request_approval`；未获批准时记录升级审批，不得把日常额度视为品类审批的替代条件。
6. 仅对非受限品类应用日常额度；金额不高于日常额度时可以记录通过。
7. 非受限票据金额超过升级阈值时，必须调用 `manager.request_approval`；未获批准时记录升级审批，不得入账。每个结论都要写明使用的票据字段、员工政策和金额依据。
